...
from config import tin_train_root_dir, tin_val_root_dir, imagenet_a_val_root_dir
...
class ImageNet_a(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform):

        super(ImageNet_a, self).__init__(root, transform)

        self.uq_idxs = np.array(range(len(self)))

    def __getitem__(self, item):

        img, label, idx, path = super().__getitem__(item)
...
def get_imagenet_a_datasets(train_transform, test_transform, train_classes=range(20),
    ...
    #train_dataset_whole = ImageNet_a(root=img_a_train_root_dir, transform=train_transform)
    ...
    test_dataset_known = ImageNet_a(root=imagenet_a_val_root_dir, transform=test_transform)
    #test_dataset_known = subsample_classes(test_dataset_known, include_classes=train_classes)

    # Get testset for unknown classes
    #test_dataset_unknown = ImageNet_a(root=tin_val_root_dir, transform=test_transform)
